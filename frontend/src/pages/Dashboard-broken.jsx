  ...
  const carregarDados = async () => {
    ...
    try {
      ...
      const [resultadoResumo, resultadoDespesas, resultadoMissoes] = await Promise.all([
        ...
        buscarDespesasPendentes(), // 🎯 CORREÇÃO: Despesas pendentes sempre próximos 15 dias
  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        {/* Saldo do Mês COM SALDO ROTATIVO */}
        <div style={{
          backgroundColor: '#ffffff',
          borderRadius: '12px',
          padding: '24px',
          boxShadow: '0 4px 16px rgba(0,0,0,0.12)',
          border: `3px solid ${(resumo?.saldoMes || 0) >= 0 ? '#34a853' : '#ea4335'}`
        }}>
          <div style={{
            fontSize: '0.875rem',
            color: (resumo?.saldoMes || 0) >= 0 ? '#34a853' : '#ea4335',
            fontWeight: '700',
            marginBottom: '8px',
            letterSpacing: '0.5px'
          }}>
          ...
          <div style={{
            ...
            {(resumo?.saldoMes || 0) >= 0 ? 'Positivo' : 'Negativo'}
          </div>
          {/* 🔄 COMPOSIÇÃO DO SALDO */}
          {resumo?.saldoRotativo > 0 && (
            <div style={{
              ...
              <div>💸 Despesas pagas: -{formatarMoeda(resumo?.totalDespesasPagas || 0)}</div>
            </div>
          )}
        </div>
